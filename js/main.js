...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Функция для отображения только онлайн-мероприятий (в Upcomingonlien-event)
    function displayOnlineEvents() {
        const onlineContainer = document.querySelector(".Upcomingonlien-event");
        if (onlineContainer) {
            const onlineEvents = eventsStore.filter(event => event.type === "online");
            onlineContainer.innerHTML = onlineEvents.map(createEventCard).join("");
        } else {
            console.error("Контейнер '.Upcomingonlien-event' не найден");
        }
    }
    

    displayAllEvents();  // Отображаем все мероприятия
    displayOnlineEvents();  // Отображаем только онлайн-мероприятия
